...
function StepperItem({
  ...
  return (
    <StepItemContext.Provider value={{ step, state, isDisabled: disabled, isLoading, tooltip }}>
      <div
        data-slot="stepper-item"
        className={cn(
          'group/step flex items-center group-data-[orientation=horizontal]/stepper:flex-col group-data-[orientation=vertical]/stepper:flex-col min-h-[80px]',
          className
        )}
        data-state={state}
...
function StepperTrigger({ asChild = false, className, children, ...props }: StepperTriggerProps) {
  ...
  return (
    <button
      ...
      className={cn(
        'focus-visible:border-ring focus-visible:ring-ring/50 inline-flex items-center rounded-full outline-none focus-visible:z-10 focus-visible:ring-[3px] disabled:pointer-events-none disabled:opacity-50',
        className
      )}
...
function StepperTitle({ className, ...props }: React.HTMLAttributes<HTMLHeadingElement>) {
  return <h3 data-slot="stepper-title" className={cn('text-xs text-center font-medium mt-2', className)} {...props} />
}
