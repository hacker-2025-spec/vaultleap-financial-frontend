...
export const filterCountries = (
  ...
  if (priorityCountries.length > 0) {
    ...
    filteredCountries = filteredCountries.filter(({ countryShortCode }) => priorityCountries.indexOf(countryShortCode) === -1)
  }

  return countriesListedFirst.length ? [...countriesListedFirst, ...filteredCountries] : filteredCountries
}

export const filterRegions = (regions: Region[], priorityRegions: string[], whitelist: string[], blacklist: string[]) => {
  ...
  if (priorityRegions.length > 0) {
    ...
    filteredRegions = filteredRegions.filter(({ shortCode }) => priorityRegions.indexOf(shortCode) === -1)
  }
